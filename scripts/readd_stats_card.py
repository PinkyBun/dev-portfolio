...
if start_idx != -1 and end_idx != -1 and "stats-card" not in html:
    ...
    print("HTML Markers not found or stats card already exists")
...
# Add stat strip CSS inside the ABOUT ME section
stat_strip_css = '''
/* Stats Card */
.stats-card {
...
if '.about-stats-strip' not in css:
    # Insert right after .about-dashboard-grid definitions
    insert_marker = '.education-card { grid-area: edu; padding: 12px 15px; }'
    if insert_marker in css:
        css = css.replace(insert_marker, insert_marker + '\n' + stat_strip_css)
    else:
        # Just append it but at a safe place, e.g., before Tech Stack Section
        idx = css.find('/* ==========================================================================\n   SKILLS / TECH STACK SECTION')
        if idx != -1:
            css = css[:idx] + stat_strip_css + '\n' + css[idx:]
        else:
            css += stat_strip_css
...
print("Properly added stats card and CSS.")
